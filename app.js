...
/* Equipment he has confirmed he does not have: no pec deck, no T-bar, no
   pulldown station, no fixed lever or sled machines. A swap list that offers one
...
const UNAVAILABLE = /^(lever-|sled-)|pulldown|smith-/;
...
if(!state.settings.programAppliedV10 && !state.session){
  try{
    ...
    state.settings.programAppliedV10 = new Date().toISOString();
    state.settings.programUpdatedAt = new Date().toISOString();
